...
public class KeyStoreHelper
{
	public static void main(String[] args) throws Exception
	{
		...
		String keyStoreFile = "D:/keystore/gotoms.jks";

		KeyStoreHelper.export("jks", keyStoreFile, password, "D:/keystore/gotoms.cer");
		...
		instream.close();
		KeyPair key = getPrivateKey(keyStore, password.toCharArray());
		Enumeration<String> aliases = keyStore.aliases();
		System.out.println(keyStore.size());
		while (aliases.hasMoreElements())
		{
			String alias = aliases.nextElement();
			Certificate ca = keyStore.getCertificate(alias);
	...
	public static KeyPair getPrivateKey(KeyStore keyStore, char[] password)
	{
		try
		{
			Enumeration<String> aliases = keyStore.aliases();
	...
	public static Certificate getPrivateCertificate(KeyStore keyStore, char[] password)
	{
		try
		{
			Enumeration<String> aliases = keyStore.aliases();
	...
	public static void export(String keyStoreType, String keyStoreFile, String password, String exportedFile) throws Exception
	{
		KeyStore keyStore = KeyStore.getInstance(keyStoreType);
		BASE64Encoder encoder = new BASE64Encoder();
		keyStore.load(new FileInputStream(keyStoreFile), password.toCharArray());
		// Key key = keyStore.getKey("privateKeyEntry", password.toCharArray());
		// Certificate caPk = keyStore.getCertificate("privateKeyEntry");
		// PublicKey publicKey = caPk.getPublicKey();

		KeyPair keyPair = getPrivateKey(keyStore, password.toCharArray());
		PrivateKey privateKey = keyPair.getPrivate();
		String encoded = encoder.encode(privateKey.getEncoded());
		// char[] encoded = encode(privateKey.getEncoded());
		FileWriter fw = new FileWriter(exportedFile);
		fw.write("-----BEGIN PRIVATE CERTIFICATE-----");
		fw.write("\n");
		fw.write(encoded);
		fw.write("\n");
		fw.write("niJs");
