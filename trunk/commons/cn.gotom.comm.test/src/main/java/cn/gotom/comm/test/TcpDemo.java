...
class TcpDemo extends JFrame
{
	...
	public TcpDemo()
	{
		...
		btnConn.addActionListener(new ActionListener()
		{
			...
			{
				TcpDemo.this.actionPerformed(e);
			}
		});
		...
		config.add(addressField);
		addressField.setColumns(10);

		JLabel lblIp = new JLabel("IP:");
		lblIp.setBounds(10, 18, 54, 15);
		config.add(lblIp);

		JLabel lblPort = new JLabel("Port:");
		lblPort.setBounds(186, 18, 54, 15);
		config.add(lblPort);
	...
	public void actionPerformed(ActionEvent e)
	{
		if (e.getSource().equals(btnConn))
		{
			...
			else
			{
				...
				client = new TcpChannel(this.addressField.getText(), port);
				try
				{
	...
	private void receiveListener(Object sender, byte[] buffer)
	{
		...
		{
			textAreaIn.setText(textAreaIn.getText() + "\n" + Converter.toHexString(buffer));
		}
		else
		{
